...
export default class selectionService {
  static async saveUserArtwork(artwork, userId, artworkId, type) {
    return axios
      .post(`${process.env.SELECTION_BACK_URL}/users/${userId}/${type}/${artworkId}`, {
        timestamp: -Date.now(),
  ...
  static async deleteUserArtwork(userId, artworkId, type) {
    return axios
      .delete(`${process.env.SELECTION_BACK_URL}/users/${userId}/${type}/${artworkId}`)
      .then(response => response.data);
  }

  static async getUserArtworksByType(userId, type, startAfter) {
    return axios
      .get(`${process.env.SELECTION_BACK_URL}/users/${userId}/${type}?limit=20&startAfter=${startAfter}`)
      .then(response => response.data);
  }

  static async getSearchResult(search) {
    return axios.get(`${process.env.SELECTION_BACK_URL}/contents/search?query=${search}`)
      .then(response => response.data);
  }

  static async getArtistAlbums(id) {
    return axios.get(`${process.env.SELECTION_BACK_URL}/contents/artists/${id}/albums`)
      .then(response => response.data)
      .catch(() => sendNotification('Erreur lors de la connexion au serveur', 'ban', 'warning'));
  }

  static async getTVShowDetail(id) {
    return axios.get(`${process.env.SELECTION_BACK_URL}/contents/tvshow/${id}`)
      .then(response => response.data)
      .catch(() => sendNotification('Erreur lors de la connexion au serveur', 'ban', 'warning'));
  }

  static async getAlbumDetail(id) {
    return axios.get(`${process.env.SELECTION_BACK_URL}/contents/album/${id}`)
      .then(response => response.data)
      .catch(() => sendNotification('Erreur lors de la connexion au serveur', 'ban', 'warning'));
  }

  static async getUserArtworksCounts(id, type) {
    return axios.get(`${process.env.SELECTION_BACK_URL}/users/${id}/${type}/count`)
      .then(response => response.data);
  }
}
